...
export async function onRequestPost(context) {
  ...
  if (action === "mint") {
    ...
    for (let i = 0; i < count; i++) {
      ...
      await kv.put("code:" + code, JSON.stringify({ test, kind, active: true, tier: (body.tier === "premium" ? "premium" : "free"), product: (body.product === "course" || body.product === "tips") ? body.product : undefined, scope: (body.scope === "reading") ? "reading" : undefined, examScope: (body.examScope ? String(body.examScope) : undefined), note: body.note || "", createdAt: Date.now() }));
      codes.push(code);
    }
